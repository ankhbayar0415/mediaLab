...
const Post = {
    ...
    return new Promise(async function (resolve, reject) {
      try {
        const [rows, fields] = await db.query(
          "SELECT * FROM `post` WHERE `post.id` EQUALS ?",
          [id]
        );
  ...
  save: async function (data) {
    ...
    return new Promise(async function (resolve, reject) {
      try {
        const [rows, fields] = await db.query("INSERT INTO `post` SET ?", {
  ...
  like: function (userId, postId) {
    return new Promise((resolve, reject) => {
      db.query("INSERT INTO post_like SET user_id = ?, post_id = ?", [
        userId,
        postId,
      ])
        .then((results, fields) => {
  ...
  unlike: function (userId, postId) {
    return new Promise((resolve, reject) => {
      db.query("DELETE FROM post_like WHERE user_id = ? AND post_id = ?", [
        userId,
        postId,
      ])
        .then((results, fields) => {
